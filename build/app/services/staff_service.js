...
export default class StaffService {
    static async list({ auth }) {
        ...
        try {
            ...
            const staffWithPermissions = staffs
                ...
                if (!isSuperAdmin) {
                    return !staff.roles.some((role) => role.slug === 'super-admin');
                }
                return true;
            });
    ...
    static async create({ request }) {
        const { fullName, email, phone, roles, password, joinDate } = await request.validateUsing(staffValidator);
    ...
    static async update({ request, params }) {
        const { fullName, email, phone, roles, joinDate, password } = await request.validateUsing(updateStaffValidator);
        const isRoleValid = await Role.query().whereIn('id', roles);
        if (isRoleValid.length !== roles.length) {
            return ResponseHelper.badRequestResponse('Terdapat peran yang tidak valid');
        }
...
//# sourceMappingURL=staff_service.js.map