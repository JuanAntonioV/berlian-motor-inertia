...
export default class TransferStockService {
  ...
  static async create({ request, auth }: HttpContext) {
    const { id, items, notes, reference, transferedAt, sourceStorageId, destinationStorageId } =
      await request.validateUsing(transferStockValidator)
    ...
    // check valid source and destination storage
    try {
    ...
    try {
      ...
      // update or create the product stock
